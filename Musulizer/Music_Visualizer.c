...
float amp(_Fcomplex z) {
    float a = crealf(z);
    float b = cimagf(z);
    return logf(a * a + b * b);
}

float amp1(_Fcomplex z) {
...
void fft(float in[], size_t stride, _Fcomplex out[], size_t n) {
    ...
    for (size_t k = 0; k < n / 2; ++k) {
        float t = (float)k / n;
        _Fcomplex v = { 0 };
        _Fcomplex temp = cexpf(_FCbuild(0, -2 * PI * t));
        v._Val[0] = temp._Val[0] * out[k + n / 2]._Val[0] - temp._Val[1] * out[k + n / 2]._Val[1];
        v._Val[1] = temp._Val[0] * out[k + n / 2]._Val[1] + out[k + n / 2]._Val[0] * temp._Val[1];
        //_Fcomplex v = mulcc(cexpf(cfromimag(-2*PI*t)), out[k + n/2]);
        _Fcomplex e = out[k];
        out[k]._Val[0] = e._Val[0] + v._Val[0];
        out[k]._Val[1] = e._Val[1] + v._Val[1];
        //out[k]       = addcc(e, v);
        out[k + n / 2]._Val[0] = e._Val[0] - v._Val[0];
        out[k + n / 2]._Val[1] = e._Val[1] - v._Val[1];
        //out[k + n/2] = subcc(e, v);
    }
}
